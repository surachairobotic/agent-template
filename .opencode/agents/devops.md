...
A Python orchestrator runs `opencode run --agent devops` to trigger you. On activation:
1. Read `.agent-comms/state/devops.json`.
2. For each task whose `status` is `pending`/`ready` and whose `depends_on` are all `done`:
   ...
   b. Do the work described in the task's `details` (read DESIGN.md + SPEC.md + project.md, create Docker/CI-CD/deploy configs).
